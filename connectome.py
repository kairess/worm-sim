...
class Brain():
    ...
    def randExcite(self):
        for i in range(40):
            try:
                name = list(self.postSynaptic.keys())[math.floor(random.random() * len(self.postSynaptic))]
                self.dendriteAccumulate(name)
            except:
                pass
    ...
    def update(self):
        ...
        if self.stimulateHungerNeurons:
            self.dendriteAccumulate("RIML")
            self.dendriteAccumulate("RIMR")
            self.dendriteAccumulate("RICL")
            self.dendriteAccumulate("RICR")
            self.runconnectome()

        if self.stimulateNoseTouchNeurons:
        ...
        if self.stimulateFoodSenseNeurons:
    ...
    def runconnectome(self):
        for ps in self.postSynaptic:
            if ps[:3] not in self.muscles and abs(self.postSynaptic[ps][self.thisState]) > self.fireThreshold:
                self.fireNeuron(ps)
    ...
    def motorcontrol(self):
        ...
        for m in range(len(self.muscleList)):
            muscleName = self.muscleList[m]

            if muscleName in self.mLeft:
                self.accumleft += self.postSynaptic[muscleName][self.nextState]
                self.postSynaptic[muscleName][self.nextState] = 0
            elif muscleName in self.mRight:
                self.accumright += self.postSynaptic[muscleName][self.nextState];
                self.postSynaptic[muscleName][self.nextState] = 0

    def setup(self):
        # The self.postSynaptic dictionary contains the accumulated weighted values as the * connectome is executed
        self.postSynaptic = {}

        # This is the full C Elegans Connectome as expresed in the form of the connectome *  neurite and the self.postSynaptic neurites.
        self.connectome = {}
